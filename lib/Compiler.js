...
class Compiler {
  ...
  resolveModulePath(modulePath) {
    ...
    if (fs.existsSync(modulePath)) {
      // 检查是否是 package.json 文件，如果是则解析其 main 字段
      if (path.basename(modulePath) === "package.json") {
        const pkg = JSON.parse(fs.readFileSync(modulePath, "utf-8"));
        const main = pkg.main || "index.js";
        const dir = path.dirname(modulePath);
        const mainPath = path.resolve(dir, main);
        // 递归解析 main 字段指定的文件
        return this.resolveModulePath(mainPath);
      }
      return modulePath;
    }

    // 如果是目录，检查目录下的 package.json
  ...
  startWatcher() {
    this.watcher = chokidar.watch(this.options.context || this.root, {
      ignored: [/node_modules/, /dist/], // 添加 dist/ 到忽略列表
      persistent: true,
    });
